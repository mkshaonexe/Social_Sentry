...
import com.example.poseexercise.posedetector.classification.EnhancedPushUpDetector
...
class PoseDetectorProcessor(
        ...
        return detector
            ...
            .continueWith(
                ...
                if (runClassification) {
                    // Use enhanced push-up detector
                    val pushUpCount = enhancedPushUpDetector.processPose(pose)
                    val currentState = enhancedPushUpDetector.currentStateString
                    val formFeedback = enhancedPushUpDetector.formFeedback
                    val isInCorrectPosition = enhancedPushUpDetector.isInCorrectPosition
                    
                    // Create enhanced result
                    val enhancedResult = PostureResult(0, pushUpCount, 
                        if (isInCorrectPosition) 1.0f else 0.5f, "pushups_enhanced")
                    classificationResult["pushups_enhanced"] = enhancedResult
    ...
    override fun onSuccess(
        ...
        // Add debug overlay for coordinate alignment testing
        graphicOverlay.add(
            DebugPoseGraphic(
                graphicOverlay,
        ...
        if (enhancedResult != null) {
            graphicOverlay.add(
                PushUpFormGraphic(
                    ...
                    poseWithClassification.pose,
                    enhancedResult.reps,
                    enhancedPushUpDetector.currentStateString,
                    enhancedPushUpDetector.formFeedback,
                    enhancedPushUpDetector.isInCorrectPosition
                )
            )
        }
    }
